...
def add_sample_data():
    ...
    if book_count == 0:
        # Add sample books; added more books and with variable amounts to assist with testing purposes
        sample_books = [
            ...
            ('Brave New World', 'Aldous Huxley', '9780060850524', 4),
...
def get_patron_borrow_history(patron_id: str) -> List[Dict]:
    """Return a full history of borrow records for a patron. Later implemented."""
    conn = get_db_connection()
    records = conn.execute('''  
        SELECT br.*, b.title, b.author 
        FROM borrow_records br 
        ...
        WHERE br.patron_id = ? 
        ORDER BY br.borrow_date
    ''', (patron_id,)).fetchall()  # stolen from pre-written function, but removing some restrictions as full history is wanted
    conn.close()
    ...
    for record in records:
        history.append({
            ...
            'status': 'Returned' if record['return_date'] else 'Borrowed'  # if returned, date is return dat is not null
        })
    return history
